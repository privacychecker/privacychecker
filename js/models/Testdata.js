  ...
  ns.TestData = Backbone.Model.extend( {
    ...
    getRateTupple: function()
    {
      ...
      while ( alreadyseen ) {
        ...
        if ( _.isUndefined( two ) || one === two) {
          alreadyseen = true;
          continue;
        }

        twoId = parseInt( two.id, 10 );
        alreadyseen = _.contains( this.alreadyComparedTuples, (oneId + twoId) );
      }

      console.debug( '[TestData] (2) Pair is ', one, '(', oneId, ') and ', two, '(', twoId, ')' );

      this.alreadyComparedTuples.push( (oneId + twoId) );
